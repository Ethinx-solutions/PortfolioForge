...
export async function setupStripeAPIV2(app: express.Application) {
  ...
  /**
   * FORGE METRICS ENDPOINT
   */
  app.get("/api/forge-metrics", async (_req: Request, res: Response) => {
    try {
      const metrics = await loadForgeMetrics();
      res.json(metrics);
    } catch (error) {
      console.error("Failed to load Forge metrics:", error);
...
export default setupStripeAPIV2;
